...
class Population {
    ...
    constructor(numInd: Int, numCd: Int, ci: Int) {
        individuals = ArrayList()
        for (x in 0 until numInd) individuals.add(Individual(numCd, ci))
    }
    ...
    val better: Individual
        get() {
            ...
            for (x in 1 until individuals.size) {
                if (individuals[x].fitnessDistance > individuals[idBetter].fitnessDistance) {
                    idBetter = x
                }
            }
    ...
    private fun ordenarPoblacionActual() {
        for (z in 1 until individuals.size) {
            for (v in 0 until individuals.size - z) {
                if (individuals[v].fitnessDistance
                    > individuals[v + 1].fitnessDistance
                ) {
